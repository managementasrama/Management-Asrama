      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            {/* Active User Badge (Clickable for Edit Account) */}
            <button
              type="button"
              onClick={() => openModal('modalAccountProfile')}
              className="flex items-center space-x-2 px-3 py-1.5 rounded-lg border text-xs bg-white/10 backdrop-blur-md border-white/10 cursor-pointer hover:ring-2 hover:ring-gold-400 transition-all text-left"
            ...
            <button
              ...
              className="p-2 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 text-gold-400 transition text-xs flex items-center justify-center shadow-xs"
              title={isDarkMode ? "Beralih ke Mode Terang (Light Mode)" : "Beralih ke Mode Gelap (Dark Mode)"}
            >
              <i className={`fa-solid ${isDarkMode ? 'fa-sun text-amber-300' : 'fa-moon text-indigo-300'} text-sm`}></i>
            </button>
